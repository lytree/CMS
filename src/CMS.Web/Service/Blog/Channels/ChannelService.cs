...
{
	private readonly IAuditBaseRepository<Channel, long> _channelRepository;
	private readonly IAuditBaseRepository<ChannelTag, long> _channelTagRepository;
	private readonly IFileRepository _fileRepository;

	public ChannelService(IAuditBaseRepository<Channel, long> channelRepository, IAuditBaseRepository<ChannelTag, long> channelTagRepository, IFileRepository fileRepository)
	{
	...
	public async Task<ChannelDto> GetAsync(long id)
	{
	...
	public async Task CreateAsync([FromBody] CreateUpdateChannelDto createChannel)
	{
		...
		if (exist)
		{
			throw new CMSException($"技术频道[{createChannel.ChannelName}]已存在");
		}
	...
	public async Task UpdateAsync(long id, CreateUpdateChannelDto updateChannel)
	{
		...
		if (exist)
		{
			throw new CMSException($"技术频道[{updateChannel.ChannelName}]已存在");
		}
