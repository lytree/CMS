...
public class UserService : ApplicationService, IUserService
{
	...
	public async Task CreateAsync(CMSUser user, List<long> groupIds, string password)
	{
		if (!string.IsNullOrEmpty(user.Username))
		{
			...
			if (isRepeatName)
			{
				throw new CMSException("用户名重复，请重新输入", ErrorCode.RepeatField);
			}
		}
	...
	public async Task ChangeStatusAsync(long id, UserStatus userStatus)
	{
		...
		{
			throw new CMSException("当前用户已处于禁止状态");
		}

		if (!user.IsActive() && userStatus == UserStatus.NotActive)
		{
			throw new CMSException("当前用户已处于激活状态");
		}
