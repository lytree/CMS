...
public class GroupService : ApplicationService, IGroupService
{
	...
	public async Task UpdateAsync(long id, UpdateGroupDto updateGroupDto)
	{
		...
		if (group.IsStatic)
		{
			...
			{
				throw new CMSException("静态权限组标识符不修改!");
			}
		}
		...
		if (anyName)
		{
			throw new CMSException($"权限组标识符:{updateGroupDto.Name}已存在!", ErrorCode.RepeatField);
		}
	...
	public async Task DeleteAsync(long id)
	{
		...
		if (linGroup.IsNull())
		{
			throw new CMSException("分组不存在，删除失败", ErrorCode.NotFound, StatusCodes.Status404NotFound);
		}

		if (linGroup.IsStatic)
		{
			throw new CMSException("无法删除静态权限组!");
		}

		bool exist = await _userGroupRepository.Select.AnyAsync(r => r.GroupId == id);
		if (exist)
		{
			throw new CMSException("分组下存在用户，不可删除", ErrorCode.Inoperable);
		}
