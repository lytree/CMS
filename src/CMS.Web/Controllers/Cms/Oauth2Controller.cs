...
public class Oauth2Controller : ControllerBase
{
	...
	public async Task<IActionResult> Home(string provider, string redirectUrl = "")
	{
		...
		if (!supportProviders.Contains(provider))
		{
			...
			throw new CMSException($"未知的privoder:{provider}！");
		}
		...
		if (authenticateResult.Principal == null) throw new CMSException("无法获取第三方授权信息");
		...
		if (user == null)
		{
			throw new CMSException("第三方登录失败！");
		}
