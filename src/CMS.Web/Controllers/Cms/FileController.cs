...
public class FileController : ControllerBase
{
	...
	public async Task<List<FileDto>> UploadFiles()
	{
		...
		{
			throw new CMSException($"最大文件数量{_fileStorageOption.NumLimit}");
		}
		long len = 0;
		...
		{
			throw new CMSException($"文件总大小{len}，超过上传文件总大小{_fileStorageOption.MaxFileSize}");
		}
	...
	public Task<FileDto> UploadAsync(IFormFile file, int key = 0)
	{
		...
		{
			throw new CMSException($"The request couldn't be processed (Error 1).");
		}
	...
	private void ValidFile(IFormFile file)
	{
		...
		{
			throw new CMSException($"不支持的文件类型");
		}
		...
		if (_fileStorageOption.Include.IsNotNullOrEmpty())
		{
			...
			{
				throw new CMSException($"不支持文件类型{ext}");
			}
			return;
		}
		...
		{
			throw new CMSException($"文件类型{ext}被禁止上传");
		}
	}
